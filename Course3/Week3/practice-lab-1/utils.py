...
def get_action(q_values, epsilon=0.0):
    ...
    if random.random() > epsilon: #random.random() generates a float between 0 and 1.
        return np.argmax(q_values.numpy()[0]) #If random.random() is greater than ε, the agent chooses the best known action. Otherwise, it chooses a random action.
    else:
...
# This function plots how well your RL agent performed over time (per episode) and optionally shows a smoothed version (rolling mean) to highlight trends. 
def plot_history(point_history, **kwargs): #point_history: a list of the total rewards (points) earned in each episode. kwargs: optional arguments to customize the plot (window size, x-axis limits, etc.).
    """
    ...
    Args:
        ...
        **kwargs: optional
            window_size (int):
                ...
                data points in point_history, i.e. if point_history has 2000 data points
                the default window size will be 200.
            lower_limit (int):
    ...
    '''
    By default:
        Plot all episodes (0 → total episodes).
        Use a rolling mean window of 10% of the data.
        Plot both raw points (cyan line) and smoothed average (magenta line).
    '''
    if kwargs:
    ...
    episode_num = [x for x in range(lower_limit, upper_limit)] #Extracts the range of scores and episode numbers you want to plot.
