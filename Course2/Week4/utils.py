...
#It’s a classic recursive decision tree construction function.
def build_tree_recursive(X, y, node_indices, branch_name, max_depth, current_depth, tree):
    '''
    Parameters:
        X: Feature matrix (shape (m, n)), where m is number of samples, n is number of features.
        y: Target vector (shape (m,)).
        node_indices: List or array of the row indices in X that belong to the current node we’re processing.
        branch_name: A string label for display purposes (e.g., "Root", "Left", "Right").
        max_depth: The maximum depth allowed for the tree.
        current_depth: Current level in the tree (root = 0).
        tree: A list used to store the structure of the tree.
    '''
